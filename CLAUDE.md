...
**Remaining before v1.0:** the final release lock (full docs sync + GDD §14 release-gate audit) at publish time, plus whatever new content/features the user wants next. Still in an expansion mood ("a lot of stuff to do") — remaining post-1.0 backlog candidates: **island-code sharing** (URL-encoded save), or **limited-time seasonal events/props** (building on the Seasons base).
