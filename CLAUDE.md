...
**v0.5 "Little Neighbours" — feature-complete & verified. v0.6 "Living Canvas" — headline breadth in, not yet feature-complete. v0.4 accepted.**
The diorama breathes. **Islanders (S16)** move in as homes appear (min Σhouse-capacity, cap 12, monotonic), **wander** a three.js-free FSM (walkable = on-island, not landmark-blocked, no solid prop; paths OK), and are **tappable** (pause → cute speech bubble + wave emote + babble). **Move-in moment**: a new resident walks in from the cloud edge with a named toast. **Pals (S18)**: cat/dog/bunny/chick/pig/cow scamper in per 8 nature items (cap 6), nibble grass, and get **petted** (hearts + chirp). Rendered by a shared **Tier-C `AgentRenderer`** (SkeletonUtils clone, mixer, idle↔walk blend, ±10% anti-sync, per-model scale-normalize) fed by pull-model snapshots. **`AssetRegistry` now keeps clips + skinned-safe clone.** **Day-night cycle (S7)**: `TimeOfDay` keyframes lights/sky/cloud-sea/fog dawn→day→dusk→night (freeze setting), **lantern glow** at night (`GlowLayer`), **fireflies + shooting stars + balloon** (`AmbientLife`), spaced **bird/cricket** ambient audio, and a **liveliness** Pops dividend. **Save v4** (islanders+pals rosters, migration). **v0.6**: **themed chunks** (Meadow/Sandbar/Spooky/Snowcap — per-chunk lawn + per-outline-point grass lip, biome preview on survey chips, `themeFor` seeded), the **Island Album** (📖/J — milestones, neighbours, pals, biome tally), **Photo mode** (P — hide UI, vignette, PNG export), and an **fps cap** setting. **129 tests green**; `verify-v04.mts` (12) + `verify-v05.mts` (10 — Islanders move in/wander/persist) both green; check+build clean.
**v0.6 still open** (honest): river/path **auto-tiling**, **edge-anchor** placement, **themed lazy-loading**, **Tiers 11–14** content (~60 items), UI-scale + colour-blind passes, theme-specific secrets/emitters & themed slab rock. Optional v0.5 polish carried: fuller move-in *set-piece* popup, sit-on-bench/observe behaviours.
...
**v0.5 is feature-complete** (Islanders + tap-to-greet, Pals + petting, move-in moment, day/night + glow, ambient life/audio, liveliness; 129 tests + both verifies green) and **v0.6 has its headline breadth** (themed biomes, Album, Photo mode, fps cap). Awaiting the user's milestone review. To **finish v0.6**: river/path **auto-tiling** + the **edge-anchor** placement rule (docks/moored boats need it), **Tiers 11–14** content (~60 items via the manifest pipeline — Harbor/Spooky/Snowcap sets), **themed lazy-loading** (S4), the UI-scale + colour-blind accessibility passes, and theme-specific secrets/ambient emitters + themed slab rock. Then **v0.7→v1.0 "Grand Opening"** (Tiers 15–20 incl. The Wonder capstone, full-arc balance, perf/browser hardening, deploy pipeline). Perf note: 20-chunk themed island + agents holds ~78–91 draws / 15–17k tris (≪ TECH §6.5 budgets); fps unmeasurable under headless swiftshader — confirm 12-wanderer/6-Pal 60 fps on real hardware (S16/S18 DoD).
