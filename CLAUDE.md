...
**v0.7 → v1.0 "Grand Opening" is underway.** Slice 1 shipped: **Tiers 15–20** (13 items — Castle/Convenience/Wonders/Dreamer; `ItemDef.tier` widened to 20, the arc now unlocks to L20), the **Pop Post** auto-collector (`autoCollectRadius` → EconomySystem radius sweep), the **Grand Assembly Hall** liveliness bonus (`livelinessBonus` → LivelinessSystem), and **The Wonder** capstone (`AuroraLayer` permanent aurora + one-time celebration + `pc.wonder` postcard). 146 tests + `verify-v07` (6) + all regressions green. **Remaining v0.7 slices:** Slice 2 (fill any Tier 9–14 content gaps), Slice 3 (full-arc L1→L20 balance/pacing sim), Slice 4 (perf/browser hardening + error-edge sweep: storage-full, clock-tamper, context-loss), Slice 5 (deploy pipeline: Pages + itch zip, og-image/favicon; docs sync; v1.0 release-gate check per GDD §14). Then **v1.0**.
