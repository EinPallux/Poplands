...
- **36-chunk soak (draw/tri budgets):** `scripts/soak-v04.mts` grows the island to the 36-chunk soft cap and asserts the hard TECH §6.5 budgets — **71 draws / 17.7k tris at 36 chunks**, far under ≤400 / ≤1.2M. Confirms the growth-aware rendering scales (≈1 draw/chunk ground + 1 merged base); + props' ≈65 pool draws (v0.2 stress) stays well under. The **60 fps** half of the gate still needs real hardware (the headless software renderer can't measure fps).
- **Deferred (not blocking the loop):** the 60 fps chunk-arrival torture test needs real hardware (the headless software renderer can't measure fps). Tutorial steps 11–12 (call-a-chunk / investigate-the-sparkle) kept as a follow-up so the shipped v0.3 tutorial stayed stable. Tier 7–10 scales are AABB-tuned first-pass estimates — a hand-polish pass may refine a few.
