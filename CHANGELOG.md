...
- **Deferred (not blocking the loop):** Tier 7+ catalog is **license-gated** — the Riverside/Harbor items depend on Pirate/Fantasy-RTS packs not yet verified in `ATTRIBUTIONS.md` (audit's blocking finding: the packs ship no license files, so provenance needs the downloader). The 60 fps chunk-arrival torture test needs real hardware. Tutorial steps 11–12 (call-a-chunk / investigate-the-sparkle) kept as a follow-up so the shipped v0.3 tutorial stayed stable.
