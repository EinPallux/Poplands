...
export const easings = {
  ...
  /**
   * Bouncier pop (ART §7.1, tuned by user feedback 2026-07-11, +bounce 07-11): a
   * damped spring that overshoots to ~1.19, settles back through a ~0.97 dip, and
   * comes to rest — one clear "boing" without the wobbliness of full elastic. A
   * linear residual correction pins the endpoints exactly: f(0)=0, f(1)=1.
   */
  popBounce: ((t) => {
    if (t <= 0) return 0;
    if (t >= 1) return 1;
